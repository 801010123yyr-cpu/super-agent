...
| 入口 | 链接 | 说明 |
| :--- | :--- | :--- |
| 官网文档 | [Super Nexus Agent 项目介绍](https://javaup.chat/super-agent/overview/project-intro) | 完整项目介绍和学习路线 |
| 在线体验 | [打开在线体验](http://super-agent-javaup.chat) | 无需本地部署，先体验核心功能 |
...
| 文档和视频 | [查看学习目录](https://javaup.chat/super-agent/overview/document-video-catalogue) | 配套文档和视频课程目录 |
| 简历写法 | [查看简历模板](https://javaup.chat/super-agent/overview/resume-template) | 项目如何写进简历、面试怎么讲 |
...
- **多通道混合检索**：PGVector 向量、Elasticsearch/BM25 关键词、结构化表格、GraphRAG 实体关系、RAPTOR 层级摘要并行召回，统一加权融合后由 Python rag-tools 精排。
...
- **文档从上传到可检索的完整链路**：Python rag-tools 负责结构化解析，Java 负责 Parent/Child 切块、向量化、关键词索引、表格索引、GraphRAG 与 RAPTOR 构建，每一步都有任务日志。
...
<details>
<summary><b>展开：学 RAG / Agent 最容易踩的坑</b></summary>
...
- 不是只用向量检索。用户问一个订单号、一个配置项名，向量检索很可能找不到。所以用了**多通道并行召回**：PGVector 向量、Elasticsearch/BM25 关键词、结构化表格、GraphRAG 实体关系、RAPTOR 层级摘要
- 各通道结果分数量纲完全不同，不能直接比大小。系统使用 rank 分、原始分归一化和元数据 boost 做加权融合
- 向量通道设了最低相似度，关键词通道用相对阈值，低于阈值的弱命中直接过滤掉
- 融合后进入 **Python rag-tools `/rerank` 精排**，在较干净的候选集上继续优化排序；旧外部 HTTP rerank 主链路已经删除
...
用户问一个订单号、一个配置项名，向量检索很可能找不到；用户问表格统计、实体关系或长文档全局总结时，单一检索通道也容易丢信息。所以 Super Nexus Agent 使用多通道并行召回：**PGVector 向量、Elasticsearch/BM25 关键词、结构化表格、GraphRAG 实体关系、RAPTOR 层级摘要**，各通道同时出发，互不阻塞。

各通道结果的分数量纲完全不同，不能直接比大小。系统用加权融合把通道 rank、原始分归一化结果和元数据命中统一到一个排序空间；向量通道设最低相似度阈值，关键词通道用相对阈值，低于阈值的弱命中直接过滤。融合后再进入 Python rag-tools `/rerank` 精排。
...
文件上传到 MinIO 对象存储后，通过 Kafka 异步触发解析任务。当前解析主链路是 `RagToolsDocumentParserService -> RagToolsClient -> Python rag-tools /document/parse`，解析产物不再只是纯文本，而是包含 `parsedText`、结构化 `blocks`、`structureNodes`、表格 HTML/行列数据、页码和 bbox 等信息。旧 Tika 主解析链路已经删除，解析失败会让任务失败并暴露问题，不自动切回旧逻辑。
...
Agent 的价值不只是能聊天，更在于能**调用外部工具完成实际任务**。Super Nexus Agent 在工具集成这块做了两层设计：
...
| :----------- | :----------------------- | :--------------------------------------------------------- |
| 检索方式     | 单路向量检索             | 多通道并行（PGVector + ES/BM25 + 表格 + GraphRAG + RAPTOR）+ 加权融合 + Python rerank |
...
[查看文档和视频目录](https://javaup.chat/super-agent/overview/document-video-catalogue)
