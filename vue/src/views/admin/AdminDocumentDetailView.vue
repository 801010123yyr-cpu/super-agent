              ...
              <div class="grid grid-cols-2 gap-3 max-md:grid-cols-1">
                <section v-for="pipeline in strategyPipelineLibrary" :key="`editor-${pipeline.key}`" class="rounded-lg border p-4" :class="pipeline.key === 'parent' ? 'border-primary/20' : 'border-[#0f766e]/20'">
                    ...
                    <div v-if="getSelectedStrategyPreview(pipeline.key).length" class="flex flex-col gap-2">
                      <template v-for="(row, rowIndex) in getSelectedStrategyRows(pipeline.key)" :key="`row-${pipeline.key}-${rowIndex}`">
                        <div class="flex items-stretch gap-2">
                          ...
                          <article v-if="row.rightItem" class="flex flex-1 items-center gap-2 rounded-md border border-border bg-card p-2.5"><div class="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-primary text-white text-xs font-bold">{{ row.rightItem.order }}</div><div class="min-w-0 flex-1"><strong class="block text-[13px]">{{ row.rightItem.label }}</strong></div><div class="flex flex-col gap-1"><button class="rounded px-1.5 py-0.5 text-[11px] font-semibold text-primary disabled:opacity-40" type="button" :disabled="row.rightItem.index===0" @click="moveStrategy(row.rightItem.type,-1,pipeline.key)">上移</button><button class="rounded px-1.5 py-0.5 text-[11px] font-semibold text-primary disabled:opacity-40" type="button" :disabled="row.rightItem.index===getSelectedStrategyPreview(pipeline.key).length-1" @click="moveStrategy(row.rightItem.type,1,pipeline.key)">下移</button></div></article>
                          <div v-else class="flex-1"></div>
                        </div>
                  ...
                  <div class="mb-3 grid gap-2" style="grid-template-columns:repeat(auto-fill,minmax(130px,1fr))">
                    <button v-for="item in strategyLibrary" :key="`${pipeline.key}-${item.type}`" class="grid gap-1 rounded-lg border p-3 text-left transition-colors" :class="getSelectedStrategyTypes(pipeline.key).includes(item.type)?'border-primary/30 bg-primary/[0.06]':'border-border bg-secondary hover:border-primary/20'" type="button" @click="toggleStrategy(item.type,pipeline.key)">
                      <div class="flex items-center justify-between gap-1"><span class="text-[11px]" :class="getSelectedStrategyTypes(pipeline.key).includes(item.type)?'text-primary':'text-muted-foreground'">{{ getSelectedStrategyTypes(pipeline.key).includes(item.type)?'已选中':'点击添加' }}</span><CheckCircleIcon v-if="getSelectedStrategyTypes(pipeline.key).includes(item.type)" class="h-4 w-4 text-primary" /></div>
